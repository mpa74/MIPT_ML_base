...
def load_tiny_imagenet(path, dtype=np.float32, subtract_mean=True):
  ...
  """
  # First load wnids
  with open(os.path.join(path, "wnids.txt")) as f:
  ...
  for i, wnid in enumerate(wnids):
    ...
    boxes_file = os.path.join(path, "train", wnid, f"{wnid}_boxes.txt")
  ...
  with open(os.path.join(path, "val", "val_annotations.txt")) as f:
    img_files = []
    val_wnids = []
    for line in f:
...
def load_imagenet_val(num=None):
  ...
  """
  imagenet_fn = os.path.join(os.path.dirname(__file__), "datasets/imagenet_val_25.npz")
  if not os.path.isfile(imagenet_fn):
    print(f"file {imagenet_fn} not found")
    print("Run the following:")
    print("cd cs231n/datasets")
    print("bash get_imagenet_val.sh")
    raise AssertionError("Need to download imagenet_val_25.npz")
